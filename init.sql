...
-- 사용자 생성 및 권한 부여
CREATE USER IF NOT EXISTS 'SAuser'@'%' IDENTIFIED BY 'password_sa';
GRANT ALL PRIVILEGES ON schedule.* TO 'SAuser'@'%';
FLUSH PRIVILEGES;
...
CREATE USER IF NOT EXISTS 'SCinsertUser'@'%' IDENTIFIED BY '${SC_P_I}';
GRANT INSERT ON schedule.* TO 'SCinsertUser'@'%';
FLUSH PRIVILEGES;

-- CREATE만 가능한 계정: SC_P_C
CREATE USER IF NOT EXISTS 'SCcreateUser'@'%' IDENTIFIED BY '${SC_P_C}';
GRANT CREATE ON schedule.* TO 'SCcreateUser'@'%';
FLUSH PRIVILEGES;

-- SELECT & SHOW 권한이 있는 계정: SC_P_SS
CREATE USER IF NOT EXISTS 'SCselectUser'@'%' IDENTIFIED BY '${SC_P_SS}';
GRANT SELECT ON schedule.* TO 'SCselectUser'@'%';
FLUSH PRIVILEGES;

-- DELETE, DROP, LOCK TABLES, SELECT 권한이 있는 계정: SC_P_DDLtS
CREATE USER IF NOT EXISTS 'SCdeleteUser'@'%' IDENTIFIED BY '${SC_P_DDLtS}';
...
CREATE USER IF NOT EXISTS 'rate_limiter'@'%' IDENTIFIED BY '${SC_P_REQ}';
...
CREATE USER IF NOT EXISTS 'SCupdateUser'@'%' IDENTIFIED BY '${SC_P_U}';
