<?php
// deleteTable.php
// 이 모듈은 flutter 앱의 요청에 따라 특정 테이블을 삭제한다.
define('ALLOW_ACCESS', true);

// 에러 보고 설정
ini_set('display_errors', 0);  // 사용자에게 에러 메시지 숨기기
ini_set('log_errors', 1);  // 오류 로그를 활성화
ini_set('error_log', '/var/log/project-schedule/delete.log');  // 오류 로그 파일의 경로 설정
error_reporting(0);
...
// 1-모임 존재 확인 -> 2-PW 검증 -> 3-테이블과 테이블 정보 삭제 모듈 실행.
// 이때 1과 2 과정이 현재 파일이다.
...
if (!empty($input['group_name']) && !empty($input['password'])) {
    ...
    if ($conn->connect_error) {  //연결을 총 1회 수행. 먼저 예외처리.
        ...
        $conn->close();  //하단의 모듈에서 $conn2 으로 다시 객체 선언함.

        //delete user로 재연결 할 예정. (하단에서 로드하는 모듈에서 연결함.)
        // 데이터 가져오기 모듈 포함
        if ($correctPass) {  //비밀번호가 일치할 때만 실행.
            include_once '__deleteTable_sqlModule.php';
