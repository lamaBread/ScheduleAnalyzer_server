...
$password = getenv('SC_P_DDLtS') ?: 'onlyDelete!';
$conn2 = new mysqli($host, "SCdeleteUser", $password, "schedule", 3307);

if ($conn2->connect_error) {
    $responseStatus = "error";  //sql connection fail
    error_log("Delete table DB connection failed: " . $conn2->connect_error);
    ...
    try {
        ...
        if ($table_name !== '__meetings_data') {
            ...
            if (!$stmt2->execute()) {
                throw new Exception("Failed to delete from __meetings_data: " . $stmt2->error);
            }
            ...
            if ($stmt2->execute()) {
                ...
                throw new Exception("Failed to delete table: " . $stmt2->error);
            }
        ...
        if (isset($stmt2)) {  // prepared statment는 트랜잭션의 종료와 상관 없이 관리해야 함.
            $stmt2->close();
        }
        // 오류 발생 시 롤백
        $conn2->rollback();
