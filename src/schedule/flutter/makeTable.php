...
if ( !empty($input['group_name']) && !empty($input['password']) && !empty($input['end_date']) && !empty($input['max_person'])) {
    ...
    $conn = new mysqli(getenv('DB_NAME'), "SCselectUser", getenv('SC_P_SS'), "schedule");
    ...
    if (!$conn->connect_error) {  //에러가 없는 경우.
        ...
        if ($lock_result) {
            ...
            if ($count > 0) {
                ...
                $conn_insert = new mysqli(getenv('DB_NAME'), "SCinsertUser", getenv('SC_P_I'), "schedule");
                ...
                if ($stmt->execute()) {
                    ...
                    $conn_create = new mysqli(getenv('DB_NAME'), "SCcreateUser", getenv('SC_P_C'), "schedule");
